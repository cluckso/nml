...
exports.createRetellAgent = createRetellAgent;
exports.createAgentAndFlowForIndustry = createAgentAndFlowForIndustry;
const client_1 = require("@prisma/client");
const prompts_1 = require("./prompts");
const plans_1 = require("./plans");
const RETELL_API_BASE = process.env.RETELL_API_BASE ?? "https://api.retellai.com";
async function createRetellAgent(data) {
    const apiKey = process.env.RETELL_API_KEY;
    if (!apiKey) {
        throw new Error("RETELL_API_KEY is not configured");
    }
    const globalPrompt = (0, prompts_1.generatePrompt)(data.businessName, data.industry, data.serviceAreas, {
        ...
        includeAppointmentCapture: data.planType ? (0, plans_1.hasAppointmentCapture)(data.planType) : false,
    });
    ...
    const voiceId = data.planType && (0, plans_1.hasBrandedVoice)(data.planType) ? "11labs-Adam" : "11labs-Chloe";
    const agentPayload = {
    ...
    const existingPhone = process.env.NODE_ENV === "development"
        ? "+14159972506"
 ...
 */
async function createAgentAndFlowForIndustry(apiKey, industry, options) {
    const businessName = options?.businessName ?? "{{BUSINESS_NAME}}";
    const serviceAreas = options?.serviceAreas ?? ["{{SERVICE_AREAS}}"];
    const globalPrompt = (0, prompts_1.generatePrompt)(businessName, industry, serviceAreas, {
...
async function createPhoneNumber(apiKey, agentId, areaCode) {
    const body = {
...
async function updatePhoneNumber(apiKey, phoneNumber, agentId) {
    const body = {
        inbound_agent_id: agentId,
        outbound_agent_id: agentId,
    };
...
async function createConversationFlow(apiKey, flow) {
    const body = {
...
async function updateConversationFlow(apiKey, conversationFlowId, flow) {
    const body = {
        model_choice: { model: "gpt-4.1", type: "cascading" },
        nodes: flow.nodes,
        start_speaker: flow.start_speaker,
...
async function updateAgent(apiKey, agentId, payload) {
...
function buildConversationFlow(businessName, industry, serviceAreas) {
    switch (industry) {
        case client_1.Industry.AUTO_REPAIR:
            return buildAutoRepairFlow(businessName);
        case client_1.Industry.CHILDCARE:
            return buildChildcareFlow(businessName);
        case client_1.Industry.GENERIC:
            return buildGenericFlow(businessName);
        case client_1.Industry.HVAC:
        case client_1.Industry.PLUMBING:
        case client_1.Industry.ELECTRICIAN:
        case client_1.Industry.HANDYMAN:
        default:
...
function buildPropertyServiceFlow(businessName, serviceAreas) {
...
function buildAutoRepairFlow(businessName) {
    return {
        start_node_id: "start-node",
        start_speaker: "agent",
        nodes: [
...
function buildChildcareFlow(businessName) {
    return {
        start_node_id: "start-node",
        start_speaker: "agent",
        nodes: [
...
function buildGenericFlow(businessName) {
    return {
        start_node_id: "start-node",
        start_speaker: "agent",
        nodes: [
