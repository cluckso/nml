/**
 * Export Owner Tips campaign to Buffer CSV, Canva CSV, ICS calendar, and playbook markdown.
 *
 * Usage:
 *   npx tsx scripts/export-owner-tips-campaign.ts
 *   npx tsx scripts/export-owner-tips-campaign.ts --start=2026-07-07
 *   npx tsx scripts/export-owner-tips-campaign.ts --tz=America/Chicago
 */
import fs from "fs"
...
import {
  ...
  OWNER_TIPS_POSTS,
  OWNER_TIPS_SCHEDULE,
} from "../lib/marketing/campaigns/owner-tips"
import type { CampaignPost } from "../lib/marketing/campaigns/types"

const __dirname = path.dirname(fileURLToPath(import.meta.url))
const OUT_DIR = path.join(__dirname, "../campaign-exports/owner-tips")

const CAMPAIGN_ID = "owner-tips"
...
function exportBufferCsv(start: string, tz: string): void {
  ...
  for (const entry of OWNER_TIPS_SCHEDULE) {
    const post = getOwnerTipsPost(entry.postId)
...
function exportGoogleBusinessCsv(start: string, tz: string): void {
  ...
  for (const entry of OWNER_TIPS_SCHEDULE) {
    ...
    const gbp =
      post.googleBusiness ??
      (slide
        ? { headline: slide.headline, body: slide.body ?? "", cta: "Learn more" }
        : null)
    if (!gbp) continue
...
function exportCanvaCarouselCsv(): void {
  ...
  for (const post of Object.values(OWNER_TIPS_POSTS)) {
...
function exportIcs(start: string, tz: string): void {
  const lines = [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//CallGrabbr//Owner Tips Campaign//EN",
    "CALSCALE:GREGORIAN",
    "METHOD:PUBLISH",
    "X-WR-CALNAME:CallGrabbr Owner Tips Campaign",
    `X-WR-TIMEZONE:${tz}`,
  ]
  ...
  for (const entry of OWNER_TIPS_SCHEDULE) {
    ...
    const uid = `owner-tips-day${entry.day}-${entry.postId}@callgrabbr.com`
...
function exportPlaybook(start: string, tz: string): void {
  const sections: string[] = [
    "# Owner Tips Campaign — 30-Day Playbook",
    "",
...
function main() {
  ...
  console.log(`Owner Tips campaign exported to ${OUT_DIR}`)
