...
type SignUpFormProps = {
  ...
  initialAgency: string | null
}

export function SignUpForm({ initialNext, initialRef, initialAgency }: SignUpFormProps) {
  ...
  useEffect(() => {
    if (initialNext) {
      try {
        sessionStorage.setItem(AUTH_NEXT_KEY, initialNext)
      } catch {
        // ignore
      }
    }
    if (initialRef) {
      try {
        sessionStorage.setItem("callgrabbr_referral", initialRef.toUpperCase())
      } catch {
    ...
    if (initialAgency) {
      try {
        sessionStorage.setItem("callgrabbr_agency", initialAgency.toUpperCase())
      } catch {
        // ignore storage errors
      }
    }
  ...
  }, [initialNext, initialRef, initialAgency])
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      // Only a session proves signup completed. Email confirmation pending (or an
      // obfuscated existing account) returns a user without a session — wait for confirm.
      if (data.session && data.user) {
        analytics.setIdentity(
          data.user.id,
          { email: data.user.email ?? emailResult.email },
