...
/**
 * Instant-proof demo: show the number first, then optionally capture contact
 * for the post-call SMS summary (consent required before SMS).
 */
  ...
  return (
    <Card className={`border-primary/50 bg-primary/5 ${className}`}>
      ...
      <CardContent className="text-center space-y-6">
        ...
        {smsReady ? (
          <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/10 p-4 text-sm text-left">
            <p className="font-medium text-foreground">You&apos;re set for a text summary</p>
            <p className="text-muted-foreground mt-1">
              After you call, we&apos;ll send one SMS with your demo lead result to {phone}.
            </p>
          </div>
        ) : showForm ? (
          <form onSubmit={handleSubmit} className="space-y-4 text-left">
            <p className="text-sm text-muted-foreground">
              Want the lead summary by text after the call? Leave your details (one message only).
            </p>
            ...
            <div className="flex items-start space-x-3 pt-2">
              ...
              >
                Yes, text me my demo result after I call — one message only. Message and data rates may
                apply.
              </Label>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              <Button type="submit" className="gap-2" disabled={loading}>
                {loading ? "Saving..." : "Text me the result"}
          ...
          <Button type="button" variant="outline" className="w-full gap-2" onClick={() => setShowForm(true)}>
            Text me the lead summary after I call
